...
def get_ffmpeg() -> str:
    ...
    try:
        ...
        if not ff:
            raise RuntimeError("找不到 ffmpeg：请 pip install imageio-ffmpeg") from None
        return ff
...
def probe_duration(path: str) -> float:
    ...
    if not os.path.exists(probe):
        ...
        r = subprocess.run(
            [ff, "-i", path], capture_output=True, text=True, encoding="utf-8", errors="replace"
        )
...
def separate_vocals(
    audio_path: str, workdir: str, device: str = "auto", model: str = "htdemucs"
) -> str:
    ...
    if exe.exists():
        cmd = [str(exe), "-n", model, "-d", device, "--shifts", "0", "-o", str(out_dir), audio_path]
    elif shutil.which("demucs"):
        cmd = ["demucs", "-n", model, "-d", device, "--shifts", "0", "-o", str(out_dir), audio_path]
    else:
        cmd = [
            sys.executable,
            "-m",
...
def rms_envelope(
    wav_path: str, sr: int = 22050, hop: int = 512, smooth: int = 11

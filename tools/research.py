...
def search_web(query: str, max_results: int = 5, proxies: dict | None = None) -> list[dict]:
...
def musicbrainz_info(title: str, artist: str) -> dict | None:
    ...
    try:
        ...
        releases = list(
            {rel.get("title", "") for rel in rec.get("releases", []) if rel.get("title")}
        )[:3]
        dates = list(
            {rel.get("date", "")[:4] for rel in rec.get("releases", []) if rel.get("date")}
        )
        return {
            "recording": rec.get("title"),
            "artist": artists,
            "albums": releases,
            "years": dates,
...
def research_package(
    title: str, artist: str, audio_path: str | None = None, proxies: dict | None = None
) -> dict:
    ...
    print(
        f"[research] 信息源: web {len(pkg['web'])} 条 | mb {'有' if pkg['musicbrainz'] else '无'} "

...
def test_compose_smoke():
    ...
    if not short_wav.exists():
        ...
        subprocess.run([get_ffmpeg(), "-y", "-i", str(short_wav), "-t", "30",
                        str(work / "short30.wav")], capture_output=True)
        short_wav.unlink()
        (work / "short30.wav").replace(short_wav)

    # 验证片：黑底白字
    events = [{"start": 1.0, "end": 6.0, "text": "对齐冒烟测试"},
              {"start": 8.0, "end": 14.0, "text": "唱完即清"}]
    from tools import ass as ass_mod
    vp, _ = ass_mod.write_two_versions(events, str(work))
    vout = work / "smoke_验证版.mp4"
    compose_mod.compose_verify(str(short_wav), vp, str(vout), str(work),
                               fps=30, encoder="nvenc")
    assert vout.stat().st_size > 100_000, "验证片过小"

    # 正式片小样：2 段 xfade（用验证版 ASS 便于肉眼比对）
    fout = work / "smoke_正式版.mp4"
    compose_mod.compose_final([str(clips_src / "mgd1.mp4"), str(clips_src / "mgd2.mp4")],
                              str(short_wav), vp, str(fout), str(work),
                              dur=3.0, xf=0.3, fps=30, encoder="nvenc")
    assert fout.stat().st_size > 500_000, "正式小样过小"
    print(f"PASS test_compose_smoke (验证 {vout.stat().st_size//1024}KB, "
...
def test_repair_loop():
    ...
    def realign(merge_gap=None, thr_low=None, thr_high=None, trim=None):
        return align_mod.align((rms, times), lines, 30.0, trim=trim or 0.0,
    ...
    assert all(b >= a for a, b in zip(starts, starts[1:])), "回退后事件须保持单调"

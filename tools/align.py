...
def detect_segments(
    ...
    thr = max(
        high * (thr_low if thr_low is not None else 0.45),
...
def align(
    vocals_wav_envelope: tuple[np.ndarray, np.ndarray],
    ...
    segs, thr = detect_segments(rms, times, merge_gap=merge_gap, thr_low=thr_low, thr_high=thr_high)
    ...
    report = {
        ...
        "thr": round(thr, 4),
        "params": {
            "merge_gap": merge_gap if merge_gap is not None else MERGE_GAP,
...
def _align_sequential(segs, lyrics):
    ...
    for i, (lrc_t, text) in enumerate(lyrics):
        ...
        if len(parts) == len(line_segs) > 1:
            for (ss, ee), part in zip(line_segs, parts, strict=True):
...
def _ev(start, end, text, src, lrc=None, delta=None):
    return {
        ...
        "text": text,
        "src": src,
        "lrc": None if lrc is None else round(float(lrc), 3),

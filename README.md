...
LLM 只在 7 个决策点出场（其余全为确定性代码），并补上了原人工流程里最大的断点——画面质检。
...
蓝色 = 记忆层。紫色 = LLM 决策点，橙色 = 人工闸门，绿色 = 独立 Agent。LLM 决策点共 7 处：
主链路 3 个（Planner 规划 / 修复循环 / 视觉质检）+ 封面 Agent 内部 4 处（调研 / 选帧 / 文案 / QC）；
背景生图为图片模型的执行调用，不计入决策点。
...
- **模态不同**：视频 Agent 驱动视频模型；封面 Agent 组合图片 + 文本 + 视觉模型，
  工具箱与失效模式完全不同。
...
衔接方式：通过文件系统交换结构化决策（`cover_decision.json` 记录每步模式与理由），
无中心对话。
...
闭环、DP 对齐算法与三路路由、修复循环与质量门槛、视觉质检、双 Agent 并行、
成本与可靠性工程、记忆设计。
